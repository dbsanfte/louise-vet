...
function renderLeisure() {
  ...
  const activities = states.map(
    (s) =>
      `<strong>${clinicAttractions[s.kind].name}</strong><p>${s.using === undefined ? 'Ready for a turn' : visitFor(s.using).name + ' is playing'} · ${s.queued} in line</p>`,
  );
...
app.addEventListener('click', (event) => {
  ...
  if (action === 'notebook') {
    ...
    if (purchase(progress, target.dataset.upgrade ?? '')) {
      simulation.build.buy(target.dataset.upgrade!);
