...
phone, to see its name in an **In the clinic** card. People include Louise and
visiting owners; pets show their owner and species, including accompanying pets.
All pet attractions, books, and customer game tables have names. During a pet’s
active attraction turn the card also describes how it feels: excited by coaster
hills, curious about the Ferris wheel’s view, cosy in the cat nook, refreshed by
a drink, or playful with toys. Each attraction has its own friendly reaction.
Queueing, boarding, returning and resting do not claim that a pet is still enjoying
a turn. An accompanying pet never inherits the patient’s reaction.

These reactions describe current activity; they do not introduce persistent mood
scores, medical benefits, preferences, fees or extra rewards. A selected pet’s
reaction updates when its turn ends. Inspecting never calls a patient, interrupts
a ride or changes the queue. Hidden fixtures and objects behind a nearer wall
or seat are not identified through that surface.
...
- **Cosy cat nook:** cats settle into a soft hooded bed, with breathing and a sleep bubble, for ten seconds.
...
home. Neighbours meeting on foot pause to chat with visible **...** bubbles and
...
with the neighbour sharing that bench using hand gestures and **...** bubbles.
...
- Reception and Hookville fill the available browser viewport, including short desktop windows
  ...
  **Patients** and **While you wait** switch the sidebar between patients and
  ...
  The **In the clinic** card shares the existing patient/activity list space,
  temporarily covering the list without changing the panel height or moving the
  camera, room jumps, paging, call or shop buttons. It shows the selected name,
  owner/role and any current pet reaction; it uses text rather than colour alone.
  Mouse information clears when the pointer leaves the scene. Touch selections
  stay readable until another tap, **Close clinic info**, Escape, a camera move,
  changing a list page/tab, a dialog or a view change. Orbit/pan drags, including
  dragging back to the starting point and two-finger gestures, never select an
  object. Empty-space taps clear the card. These temporary selections are not saved.
  A tap remembers the pet under the finger when it starts, even if a ride carries
  the pet away before release.
...
Clinic identity acceptance: visible pets, accompanying animals, owners, Louise
and all built attractions reveal the correct name on mouse hover or touch. An
active rider shows its attraction’s reaction; a companion or waiting pet does not.
Reactions clear when turns end, hidden fixtures cannot be picked, and camera
movement/view changes/dismissal release the card. Inspection does not call pets,
change scores or move controls below the viewport, including a full clinic on a
360 × 640 phone.
...
| Clinic names and activity reactions      | [src/clinic-identity.ts](../src/clinic-identity.ts), [src/clinic-inspection.ts](../src/clinic-inspection.ts), [src/town.ts](../src/town.ts)            | [clinic-identity.spec.ts](../tests/clinic-identity.spec.ts), [leisure.test.ts](../tests/leisure.test.ts)                      |
