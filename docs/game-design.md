...
zero available is greyed out and cannot place anything. Unowned types remain
visible with zero availability and a reminder to buy a copy in Shop.
...
**Room**, **Garden** and **Remove floor** draw a rectangle with a drag or two
opposite corners. A preview shows its area, price, and a green or red footprint.
...
and zoom remain available with the Camera tool, arrow keys and pan/zoom buttons.
Separate left/right camera-turn buttons are always available, including while
placing furniture; turning the camera does not rotate the selected item.
...
Only the collection scrolls internally; feedback,
...
- Reception and Hookville fill the available browser viewport, including short desktop windows
  ...
  **Patients** and **While you wait** switch the sidebar between patients and
  ...
  the wheel zooms; two fingers pan and pinch to zoom. Arrow keys and eight visible
  camera buttons provide alternatives: left/right turn, four pan directions and
  zoom in/out. Camera-turn buttons rotate by 22.5 degrees around the current
  target, keeping its position, tilt and zoom. They work in office and Build on
  desktop/touch, separately from furniture rotation.
...
Build with a model preview and distinct available/placed counts; zero-available
placement buttons stay grey and disabled. Move placed cycles through existing
...
fixed editor tools and both camera-turn buttons stay separate and within the
viewport on small phones and short desktop windows. Turning the camera preserves
the layout, selected item's orientation, target and zoom. Satisfaction/advertising
...
cutaway boundary. Shop scrolling remains stable and phone users can always reach
