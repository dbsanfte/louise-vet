...
Floor edits retain the permanent divider and doorway between reception and this
room. When building behind reception beside the fixed suite, only the old exterior
stretch and its window give way to the new space’s boundaries and connections.

### Growing the clinic and spending time together

**Build** opens a Sims-style grid editor from the office. It is the improvement
stage of the care → reward → reinvestment loop: children choose the shape of their
clinic and find their own places for its furniture and pet amusements. Town time
pauses while building. Finish or stop a visit before entering; cancel a pending
...
Two prominent mode buttons separate **Place furniture** from **Build spaces**.
...
has enough coins. Furniture has no room-kit prerequisite; custom-built rooms and
gardens can use every suitable item. Starter furnishings can also be bought
...
using **Place furniture**. There are no Move placed buttons or zero-stock entries in
Build. Shop still shows placed/stored totals when buying another copy. Purchased
room kits with unused floor tiles appear first among the matching surface's
prefabs in Build spaces. Their cards show the remaining allowance and cost;
tiles stay a shared allowance, displayed against kits in purchase order. Partly
used kits keep their credit label; fully used kits become ordinary reusable floor
plans. Legacy rooms never grant their floor credits twice.
...
edits save immediately; cancelling does not undo earlier accepted edits.

**Build spaces** offers **Floor** and **Garden** surfaces. Furniture defines how
an area is used; there is no furnishing checklist or assigned room type. Boundaries
and connections follow the surfaces automatically, with no Interior/Exterior
buttons. Garden additions join existing gardens openly, removing the old fence
along their shared edge. The outer lawn remains fenced. Indoor additions keep
low walls; connections between rooms or from a garden to a room get clear doors
where needed. A garden already reachable through another garden does not need
an extra door through every adjoining indoor wall. Existing partitions elsewhere
stay as saved and can still be edited with **Doors**.

Draw and release on desktop and touch screens:

1. Drag between two grid corners, or tap the first and opposite corners. The
   first corner stays fixed; the translucent footprint and outline update while
   held. Corners snap to whole grid edges and nearby existing boundaries (within
   0.8 tile). Width, depth and cost update beside the scene. Invalid rectangles
   are red and explain the obstruction; a narrow stroke makes a one-tile strip.
2. Releasing a valid drag or tapping the second valid corner **builds immediately**.
   Floor, walls/fences and automatic doorways commit and save together, charging
   once. There is no Build space confirmation button. Openings join old and new
   floor at their actual shared edges, including overlapping rectangles. Existing
   surfaces remain intact and only new tiles cost coins or credits. Separate added
   patches each get a reachable connection. Green doorway hints appear while drawing
   where doors are needed; open garden joins have no doorway hint.
3. **Undo**, shown as a back arrow in the controls row, reverses consecutive space
   edits in the current editing session, including prefabs, free repainting,
   floor erasure and doorway changes.
   It restores the exact preceding floor, walls, doors and credits and returns
   only the coins charged by that edit. Undo saves immediately and is disabled
   when there is nothing to reverse. Ctrl/Cmd+Z is its desktop shortcut. Camera
   movement and switching between Floor/Garden preserve this history. Picking
   furniture, opening Shop, leaving Build or reloading
   starts a new space history; Undo never reverses later purchases or furniture edits.

An invalid release builds nothing and spends nothing. Its red outline remains as
feedback; the next stroke replaces it directly, or Cancel clears it. Cancel,
Escape, capture loss, releasing outside the canvas and a second finger interrupt
unfinished strokes without charging. Two fingers always pan/pinch/twist the Build
camera; the remaining finger cannot accidentally build after a gesture. A desktop
keyboard alternative uses the nudge arrows and **C** to set each corner; the second
corner builds immediately. Erasure instead opens a Yes/No confirmation on release.

Build spaces also offers four reusable pictured **floor plans**: Waiting room
...
and boundary. Both **Garden** and **Pet playground** pictures lead the Garden
catalogue, above instructions, so outdoor prefabs are visible immediately on phones.
Floor shows Waiting room and Adventure room; unused kit allowances are listed first. Drag a picture into the scene for a live green/red preview; a valid
drop builds immediately. Alternatively, tap a card, then tap a clear scene spot
or use **Rotate** and **Place space** (plus nudge controls on desktop). Plans snap to whole
...
Prefabs use the same automatic room/garden connections as drawn rectangles.
Doorways can be changed afterward.

Prefabs are starting shapes, not furnished room purchases or locked room types.
All remain reusable in the Floor/Garden guide, independent of kit ownership; they
...
ordinary saved construction, customizable using the same Floor, Garden, Doors,
Erase and Place furniture controls. There is no whole-room move tool.
...
partition section. Doorways and adjustments cost no extra coins. Every space must
keep a reachable entrance; occupied doorways and furniture prevent unsafe closing.
**Erase** accepts rectangles over furnished and occupied space. Its live preview
stays valid while drawing; releasing the mouse/finger, or tapping the second
corner, opens **Erase this space?** with the tile count and a grouped list of
furniture to return. **No, keep it** or Escape discards the plan without changing
the clinic, occupants, wallet or save. **Yes, erase space** removes the selected
removable floor and stores every furniture copy whose footprint overlaps the
rectangle or loses its supporting floor/approach. Returned furniture stays owned
and appears in Place furniture with its available count; this is an inventory
return, not a coin sale or another purchase. Blank ground and protected clinical
floor are ignored. Erasure has no construction-size limit and an empty selection
does nothing.

Remaining indoor edges close with walls and garden edges with fences. Doorways
whose adjoining floor vanished are removed; safe replacement doors connect the
remaining pieces wherever shared floor still exists. A fully separated leftover
stays in place, with its furniture inactive until a connection is restored.
Building floor back against a retained piece automatically opens safe doors as
needed and reactivates its furniture; unrelated detached pieces remain inactive. No tiles outside the rectangle are silently deleted or filled in.
Owners and pets whose floor, seat, ride or route is affected move to separate,
reachable ground in the usable clinic during the paused Build edit. Ride users
return to a ground-level rest state, and companions discard obsolete following
paths. Occupants never veto erasure or remain stranded. The confirmed geometry,
collection and safe activity state save together. Undo restores the exact floor,
walls, doors and furniture without duplicating copies; occupants are moved clear
again if restored furniture would overlap them. The Yes/No actions stay visible
on desktop, portrait and landscape phones, with only long explanatory lists
scrolling inside the focused dialog.

Room floor has low cutaway walls and open door leaves; garden floor has grass,
cream picket fences and gates. Editing the floor or doorways replaces the old
modular shell with the saved boundaries; moving furniture alone retains migrated
architecture. Existing shared partitions and openings migrate when first edited;
old custom open-plan layouts keep their floor and paid furniture. Version 3 of the
build state saves wall segments and doorways alongside the existing floor and
collection, accepting versions 1 and 2. Its optional detached-floor list preserves
leftovers created by erasure; reconnected patches leave that list. Legacy future-room doors appear only in
walls of rooms that already exist. The public front entrance, counter, examination
suite and Louise's escort route remain anchored and protected. Arbitrary standalone
wall drawing, upper floors and movable treatment rooms remain outside this editor.
...
Only the collection or space guide scrolls internally; feedback,
mode switches, Undo/Erase, Rotate, Place item/space, Cancel, Shop and Done stay
in the viewport on phones.
Entering Build or closing its Shop clears any floating purchase confirmation so
it cannot cover the editor's feedback or controls.

Placement checks reject overlapping solid furniture, missing supporting floor,
blocked seats/attractions, occupied ground and blocked clinical access, with a
...
places even if all chairs are stored. Families remain callable afterwards.

Existing saves migrate all owned furniture, original room footprints, active
waiting activities and exact positions without charging, rearranging, or removing
anything. Opening Build does not customize the clinic. The original usable bench,
treat shelf, plants, rugs and picture join the collection too. Room upgrades now
supply optional floor-tile kits without unlocking shop categories, while retaining the original
capacity progression from four to six to eight patients. Existing owned rooms
remain built; they do not also receive a second free tile allowance. Room buttons
remain quick jumps to built areas at their original locations.
...
The pet playground is a contained amusement garden. Migrated clinics retain the
six-by-fourteen-unit original footprint and side aisle, clear of Willow Crescent's
pavement. In Build, players can reshape and extend it across available grounds and
move its amusements into any suitable built space. The kit's
...
A reply follows the owner's five-second bubble and quiet gap only while both
remain visible in the same action. Inspection or an action change cancels it.
Attraction information uses a rounded label without a speaking tail.
...
Pets and owners also react automatically during their actual activities. Coaster
riders think “Wheee!”, cats enjoy a scratch or a cosy rest, and pets at the treat
...
Messages are authored as complete, situation-specific sentences in clear,
everyday English. Keep playfulness concrete: a pet enjoys a crunchy treat, wants
another turn or wonders where its owner went. Avoid forced name puns, baby talk,
...
The **Play garden extension** supplies an optional floor-tile kit. The following
activities can be bought separately without it. Migrated clinics keep their existing six-by-eight garden
behind the lounge. Each activity can be placed and oriented in Build:
...
The extension adds activity space without changing patient capacity.
Buy, place and rearrange these items using Build, with no room kit required. Dispenser refills are included and do not consume retail treat stock.
...
The optional **Sunshine courtyard** kit leaves patient capacity unchanged.
The shop offers a two-seat **Puzzle picnic table** plus three pet
diversions. Migrated clinics keep the six-by-eight lawn behind the playground,
cream pointed pickets, stepping stones and clear side-aisle doorway. Build can
reshape this outdoor area, extend it into the remaining greenspace, or replace
parts with room floor. Its activities are:
...
its place to reception and checks in; departing families clear the doorway first.

Outside families wait **60–88 active seconds**, with individual variation. If
still unable to enter, they release their pavement place and decide, “Hmm, it is
very busy. We will come back later.” Their departure feeling remains eligible
for twelve active seconds, and Town news records the deferred visit. Routine
checkup/vaccination families resume their park outing; other families first take
their patient home. Unwell pets rest in their garden while owners and healthy
companions resume normal walks, chats and park visits. There is no coin or
satisfaction penalty. Families already inside, called or being examined never
leave because of this timer.

The original ticket, patient, care reason and any accompanying rescue patients
remain saved. After **3–5 active minutes**, a family may return from home only
when the clinic has room for its journey; oldest eligible retries are considered
first. A full clinic therefore does not keep sending the same families back to
the pavement. Return times and remaining outside patience survive reloads;
older crowded saves gain patience when their recovered route reaches a place.
Deferring never counts as treatment, grants rewards or clears an injury. Reading
care, dialogs and hidden tabs retain their normal pause behavior, so this is an
ambient town routine rather than a treatment deadline. Other families' rescue
stories can continue while deferred patients await their own care.

After care, families walk from their indoor position back through that
...
BPM and sound control occupy a dedicated instrument dock, separate from the
patient, tools, notebook and action bar. They stay visible as clues accumulate
and when the device rotates. The chest-placement prompt remains in the
...
- Keep the care notebook and visit actions within the viewport. On desktop the
  notebook docks above the action bar; compact visits give it a separate grid area.
...
- Reception and Hookville fill the available browser viewport, including short desktop windows
  and phones, with Build, Clinic shop, Hookville, My clinic and Care notebook always in
  ...
  **Patients** and **Who’s waiting** switch the sidebar between patients and
  ...
  the wheel zooms; two fingers pan and pinch to zoom. Desktop players have arrow
  keys and eight camera buttons: left/right turn, four pan directions and zoom
  in/out. Camera-turn buttons rotate by 22.5 degrees around the current target,
  ...
  block camera gestures or patient selection. Speech wedges track the animated mouth;
  thought-cloud circles point above the head. Names are bold and messages regular;
  positions stay within the scene, and labels hide while their target is hidden
  or offscreen. If cropping leaves no room between the bubble and its anchor,
  omit the pointer rather than drawing it through the text. On very short phones the bubble can briefly overlap scene
  ...
  **Who’s waiting** lists active rides,
...
- Examinations up to 1200px wide use a single screen-sized layout, with no browser
  page scrolling. Portrait phones stack the patient, instrument dock, tools,
  notebook and visit actions. Tablets use a side column; short landscape phones
  put tools/readings and notes/body guides on either side of the patient. Rotation
  preserves the visit, selected tool, contact and observations. Camera controls
  and labelled body guides have their own space outside the canvas; instrument
  viewers scale to both canvas dimensions. The ECG, magnification/whole-body
  controls, notes and buttons never cover one another.
- Keep every available sick-visit tool visible together. Compact routine visits
  show the two species-specific standard-check buttons instead of a duplicate
  general tool palette. Diagnosis choices and care tools occupy the same stable
  control area. **Visit info** opens the owner's story and visit steps for
  unhurried reading; closing it resumes the same visit. Compact feedback appears
  inside the notebook (or vaccination care plan), with no floating toast covering
  controls. A new observation opens Care notes; the tabs restore notes/clues after
  guidance. Only notebook content and longer dialog text scroll internally.
  Vaccination retains its placement/skill flow, and Stop visit stays available.
  These layout changes do not alter clues, rewards or saved progress.
...
| Toy corner        | 60            | One extra toy attraction.                                      |
...
All shop items are available without prerequisite kits. Kits can be bought in any
order and never reduce an existing capacity increase. Insufficient coins or a second purchase of a one-time
upgrade leaves both the wallet and collection unchanged. Retail currently happens
...
The construction rectangle limit is 250 tiles per action; erasure can cover a
larger area because it only removes existing editable tiles. Changing an existing tile's surface
...
neither coins nor kit credit. Confirmed erasure returns affected furniture to
inventory for free, preserving each paid copy and all upgrade bonuses. Undoing
erasure restores its placements without a coin transaction. Undoing a space build restores its spent coins and
floor credits exactly; an invalid or cancelled stroke spends nothing. Stored items can be
...
results and Build pause town time. Browsing preserves the same records and queue. Indoor families wait without a deadline; outdoor queues use the deferred-care return-later behavior described in the visit loop, with no score penalty. Advertising creates opportunities, not a deadline.
...
the snapshot. Wallet and purchased copies save together. If unrelated town activity
fails validation at startup, a separately validated Build layout is retained while
the town restarts. Recovery keeps every copy, placement and unused floor credit;
it does not reconstruct the collection from one-time upgrade flags. An invalid
Build layout itself is never applied. Older snapshots without this state are migrated from owned upgrades,
...
An erasure plan or confirmation is temporary and spends/changes nothing until Yes.
Confirmed erasure saves the floor, returned collection and relocated occupants
together; version 3 also retains explicitly detached floor patches until connected.
Invalid layout data rejects the town snapshot atomically.
...
| V2-04 | Town-driven clinic visits                  | Implemented | Shared actors/positions, including accompanying pets, drive pavement → doorway → waiting place → care → doorway → home, with regular care, abort/requeue, one-time rewards and saved state. Full-clinic families reserve separate exterior places, enter in arrival order and leave the doorway clear for departures. Outside waits expire after 60–88 active seconds; saved care retries after 3–5 minutes only when space opens, keeping families circulating without losing care. [Simulation checks](../tests/community.test.ts), [browser journeys](../tests/community.spec.ts), [entrance and save checks](../tests/entrance.test.ts), [rendered entrance](../tests/entrance.spec.ts).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| V2-13 | Grid construction and furniture collection | Implemented | Room/garden grid expansion across available clinic grounds, separate Place furniture/Build spaces modes, pictured spare-only furniture inventory and surface-specific prefab grids with unused kit credits first, one card per stored type, available counts and direct scene pickup for existing furniture, stored shop items, quarter-turn placement, picking/moving/storing, occupied-item unloading, custom routes, exact legacy migration, saved layouts and recovery of paid furniture when town activity is rejected. Live snapped rectangle previews on desktop/touch, release-to-build custom rectangles with fixed anchors, nearby edge snapping, overlap-aware boundaries and automatic safe entrances for every added patch, dedicated back-arrow Undo/eraser controls and exact space refunds, furnished erasure with release-time Yes/No confirmation, inventory returns, safe occupant relocation, repaired boundaries/doors and saved inactive leftovers, invalid-stroke retry without spending, automatic surface-aware garden joins and room doors without boundary toggles, reusable pictured drag/drop prefabs, two-finger pan/pinch/twist in every Build tool, flexible room/garden guides, direct furniture purchases without room-kit gates, shared doorway placement/closing and wall-section removal, saved walls and route safety. Counter, entrance and treatment suite anchored; paid copies with independent seating/queues/animations and no per-type ownership cap. No standalone wall drawing, upper floors or movable clinical suites. [Layout and unloading checks](../tests/clinic-build.test.ts), [desktop/touch editor checks](../tests/clinic-build.spec.ts). |
...
- Reception supports desktop/touch orbit, pan and zoom plus desktop keyboard and
  ...
  existing entrance and activity routes unobstructed. Desktop camera buttons and
  room shortcuts fit short windows without overlap or browser scrolling. Phones
  and tablets keep room shortcuts visible in portrait and landscape.
...
older crowded saves recover without losing patients; neglected outside queues disperse
into daily routines, preserve cases/companion care across reload and return only
after their cooldown when room opens, while indoor patients keep their places;
...
Modular clinic acceptance: every furnishing can be bought without a room kit and
charges for each copy; optional room kits charge once, in any order;
...
action. Names are bold and message text regular. Human speech points to the
animated mouth; animal feelings form thought clouds with shrinking head circles.
Animal spoken replies contain only species-appropriate noises and no fish speech.
...
must not obscure anatomical targeting, the notebook, or primary actions on
desktop or touch screens. Compact visits fit 360 × 640 phones, short landscape
phones and tablets without browser scrolling: all tools and body guides are
reachable, ECG/optical controls occupy their own dock, and rotation preserves
contact and observations. Visit info, diagnosis, vaccination, care cancellation
and Stop visit remain usable. After multiple observations, the active notebook page
...
Rejecting malformed town activity must still retain a valid paid collection and
layout, including stored copies and unused floor credits, through later reloads.
...
Purchased room kits with remaining tile allowance lead their matching surface
prefabs in Build spaces and open the pictured plan without spending or changing the layout until placement. Remaining
...
Owners vacate moved seating and elevated pets finish at ground level before a
...
depth and committed tiles share the same grid-corner bounds. A valid release or
second corner tap builds immediately, charges once, and adds required connecting doors;
no Build space confirmation is present. Repeated Undo restores each preceding
layout and its exact coins and floor credits, including prefabs, overlapping floor,
free repainting and migrated boundaries. Undo saves correctly and cannot refund twice
or remove subsequent purchases/furniture. Separate mode buttons, surface choices,
back-arrow Undo and eraser controls remain visible and distinct on phones and desktop.
Both Garden prefab pictures appear above instructions and within the initial visible
catalogue. No Interior/Exterior buttons remain. Garden-to-garden extensions remove
only their shared fence and merge openly; garden-to-room and room-to-room additions
keep boundaries with safe doors as needed. A mixed garden/room boundary needs no
redundant door when an open garden route already works. Fresh, migrated and
reloaded layouts, overlapping rectangles and several occupied boundary positions
retain continuous routes. Drawing a bridge to a retained room restores its access
without a separate door edit. Furniture bought without any kit can be placed, used,
stored and reloaded on custom floor; insufficient funds and unsafe placement still
fail without spending or losing copies.
A second finger, cancellation, capture loss and release outside the canvas interrupt
a stroke without building. Invalid or unaffordable rectangles stay red, spend nothing,
and can be redrawn directly. Camera gestures preserve completed construction and
its Undo history. Pictured prefabs preview before release, charge once on valid drop,
and reject overlap or disconnected placement. Invalid drops remain retryable;
dropping outside cancels. Built and undone layouts survive reload. Furnished and
occupied erasure previews are valid. Release opens a focused Yes/No dialog with
counts; No and Escape preserve the complete snapshot and persisted save. Yes
returns overlapping/unsupported furniture copies exactly once, keeps coins and
credits unchanged, repairs remaining boundaries and doors, and safely relocates
owners, ride users and companion pets. Detached leftovers stay saved and inactive
until reconnected. Confirmed erasure and Undo survive reload without lost/duplicate
items or stranded actors; subsequent care, Stop visit and departure still work.
The confirmation actions fit portrait and short landscape phones without page scrolling.
...
Rotate, Undo, Erase, Place item/space, Cancel and Done without page scrolling.
...
further construction choices remain open beyond the implemented grid editor.
...
- Should Build next offer standalone interior-wall drawing, movable treatment rooms, or additional floors?
...
Space construction now uses draw → release to build → Undo, superseding the
separate confirmation step. Undo restores paid coins, kit credits and geometry
instead of making children confirm every valid rectangle. Prominent Place furniture
and Build spaces modes separate furnishings from construction; compact pictured
Floor/Garden prefabs lead their lists, and a dedicated controls row holds Undo/Erase.
Surface-aware connections supersede the Interior/Exterior controls: children
choose Floor or Garden, and the builder joins gardens openly and adds necessary
room doors. Optional kits no longer gate shop items, so custom construction can
use the full furniture catalogue. Fixed anchors, nearby snapping and
overlap-aware connections preserve the full rectangle. Two-finger camera controls
remain available in every tool; the clinical core stays anchored.
Erasure uses a release-time Yes/No prompt because one stroke can return multiple
furnishings. The editor moves occupants to safety and retains separated leftovers
so children can reshape their clinic without dismantling every item first.
...
access and automatic unloading keep rearrangement safe for waiting families.
...
| Visit stages, queue, actions, timing, UI | [src/main.ts](../src/main.ts), [src/style.css](../src/style.css), [compact layout](../src/exam-layout.ts), [compact styles](../src/examination-layout.css)                                                                                                   | [clinic.spec.ts](../tests/clinic.spec.ts), [exam-layout.spec.ts](../tests/exam-layout.spec.ts)                                |
| Reception, cameras, targets, rendering   | [src/world.ts](../src/world.ts)                                                                                                                                                                                                                              | [clinic.spec.ts](../tests/clinic.spec.ts), visual inspection                                                                  |
| Clinic names and activity reactions      | [identity](../src/clinic-identity.ts), [bubbles](../src/world-bubble.ts), [voices](../src/character-voices.ts), [dialogue](../src/character-dialogue.ts), [events](../src/character-feelings.ts), [pet calls](../src/pet-replies.ts), [town](../src/town.ts) | [clinic-identity.spec.ts](../tests/clinic-identity.spec.ts), [leisure.test.ts](../tests/leisure.test.ts)                      |
