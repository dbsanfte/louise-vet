...
`public/images/catalogue/*.webp` contains 46 original 256-pixel item portraits rendered
...
dropping a prefab builds. This mode leaves the 46 item pictures unchanged.
...
The eight additional amusement GLBs (`fish-bubbles`, `fish-reef`, `agility-tunnel`,
`snuffle-mat`, `cat-feather`, `dig-box`, `bird-hoops`, `pet-piano`) and `bowl-trolley`
are original project models generated by `scripts/create-extra-amusements.py`.
The editable source is `assets/blender/extra-amusements.blend`; reproduce this
scope with `python3 scripts/blender-build.py --headless --amusements`. Named
feather, key, bubble and wheel groups provide runtime animation. Their eight
WebP shop/build pictures are rendered from these models by
`scripts/render-catalogue.mjs --only=fish-bubbles,fish-reef,agility-tunnel,snuffle-mat,cat-feather,dig-box,bird-hoops,pet-piano`.
`src/fish-enrichment.ts` supplies original submerged bubbles, hoop geometry and
swimming motion inside the existing fish bowl. The piano’s optional quiet notes are synthesized by the existing `src/audio.ts` sound system. There are no external assets or
third-party licenses in this addition.

`src/care-scene.ts` reuses the existing Blender animals, mouth anatomy and care
instruments. Its original procedural contact patches, hopping fleas, drops and
three-dimensional paw bandage are runtime geometry, with no external images.
